     ...
     <div class="container">
         ...
         <thead>
             ...
             <thbody>
                 ...
                 if($search == ""){
                 $sql = "SELECT * FROM `rentbook` INNER JOIN member ON rentbook.mem_id = member.mem_id INNER JOIN book ON rentbook.book_id = book.book_id INNER JOIN rent_status ON rentbook.rent_status_id = rent_status.rent_status_id WHERE member.mem_id = ".$_GET['id']." ORDER BY `rentbook`.`rent_id` DESC " ;
                 $ans = $conn->query($sql) ;
                 $row = $ans->num_rows ;     
                  }
                 else {
